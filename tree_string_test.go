...
func TestTreeString(t *testing.T) {
	// n belongs to [a, b)
	n := a + rand.Intn(b-a)
	els := randStringSlice(n)
	tree := NewTreeString(els...)
...
func (p *nodeString) isSorted() bool {
	if p == nil {
		return true
	}
...
func (t *TreeString) isSorted() bool {
	return t.root.isSorted()
}

func (p *nodeString) isBalanced() bool {
	if p == nil {
		return true
	}
	bf := p.balanceFactor()
...
func (t *TreeString) isBalanced() bool {
...
func BenchmarkTreeString(b *testing.B) {
	...
	for idx, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				tree := NewTreeString(dataSetsString[idx]...)
