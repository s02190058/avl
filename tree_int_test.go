...
func (t *TreeInt) isSorted() bool {
	return t.root.isSorted()
}

func (p *nodeInt) isBalanced() bool {
	if p == nil {
		return true
	}
	bf := p.balanceFactor()
...
func (t *TreeInt) isBalanced() bool {
	return t.root.isBalanced()
}

func BenchmarkTreeInt(b *testing.B) {
	benchCases := []struct {
		name string
	}{
		{
	...
	for idx, bc := range benchCases {
		b.Run(bc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				tree := NewTreeInt(dataSetsInt[idx]...)
